...
<?php if(!$error) { ?>
	<div class="buttons">
		<div class="right"><a id="button-pay" class="button"><span><?php echo $button_betacoin_pay; ?></span></a></div>
	</div>
<?php } else { ?>
	<div class="warning">
		<?php echo $error_msg; ?>
...
var timeleft = <?php echo $betacoin_countdown_timer; ?>;
var checker = 0;
...
$('#button-pay').on('click', function() {
	if(timeleft > 0) {
		...
		html += '		<div style="font-size:16px; padding:6px; text-align:center;"><?php echo $text_please_send ?> <span style="font-size:18px; border-style:solid; border-width: 1px; border-radius:3px; padding-top:3px; padding-right:6px; padding-left:6px; padding-bottom:3px;"><?php echo $betacoin_total; ?></span> <?php echo $text_btc_to ?> </div>';
		html += '		<div style="font-size:16px; padding:6px; text-align:center;"><span style="font-size:18px; border-style:solid; border-width: 1px; border-radius:3px; padding-top:3px; padding-right:6px; padding-left:6px; padding-bottom:3px;"><?php echo $betacoin_send_address; ?></span></div>';
		html += '		<div style="font-size:16px; padding:6px; text-align:center;"> <?php echo $text_to_complete ?></div>';
		html += '		<div style="font-size:16px; padding:6px; text-align:center;"><a style="font-size: 16px;" href="betacoin:<?php echo $betacoin_send_address; ?>?amount=<?php echo $betacoin_total; ?>"><?php echo $text_click_pay ?></a> <?php echo $text_uri_compatible ?></div>';
		html += '	</div>';
		html += '<div id="payment-right" style="float: right;"><img src="http://chart.apis.google.com/chart?cht=qr&chl=betacoin:<?php echo $betacoin_send_address; ?>?amount=<?php echo $betacoin_total; ?>&chs=160x160"></div></div>';
	...
	$.colorbox({
		...
		html: html,
		onComplete: function() {
			checker = setInterval(betacoin_check, 5000);
			$('#button-confirm').on('click', function() {
				$.ajax({ 
					type: 'GET',
					url: 'index.php?route=payment/betacoin/confirm_sent',
					timeout: 5000,
					dataType: 'text',
					error: function() {
