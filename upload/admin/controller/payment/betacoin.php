...
class ControllerPaymentBetacoin extends Controller {
	...
	private function validate() {
        ...
        if (!$this->request->post['betacoin_rpc_username']) {
            $this->error['username'] = $this->language->get('error_username');
        }
        if (!$this->request->post['betacoin_rpc_address']) {
            $this->error['address'] = $this->language->get('error_address');
        }
        if (!$this->request->post['betacoin_rpc_password']) {
            $this->error['password'] = $this->language->get('error_password');
        }
        if (!$this->request->post['betacoin_rpc_port']) {
            $this->error['port'] = $this->language->get('error_port');
        }
        if (!$this->request->post['betacoin_prefix']) {
            $this->error['prefix'] = $this->language->get('error_prefix');
        }
        if (!$this->request->post['betacoin_countdown_timer']) {
	...
	public function index() {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('payment/betacoin', 'token=' . $this->session->data['token'], 'SSL'),
            'separator' => ' :: '
        );
		...
		$rows = $result->rows;
		$max = $result->num_rows;
		$betacoin_total_in_db = 0;
		$betacoin_address_in_db = 0;
		for($i = 0;$i < $max;$i++) {
			if($rows[$i]["Field"] == "betacoin_total") {
				$betacoin_total_in_db = 1;
			}
			if($rows[$i]["Field"] == "betacoin_address") {
				$betacoin_address_in_db = 1;
			}
		}
		if(!$betacoin_total_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD betacoin_total DOUBLE AFTER currency_value;");
		}
		if(!$betacoin_address_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD betacoin_address VARCHAR(34) AFTER betacoin_total;");
		}
